...
func TestPermissionRepositoryWithContainer(t *testing.T) {
	...
	t.Run("Test permission creation", func(t *testing.T) {
		...
		roleCreated, err := permissionRepo.StorePermission(permission)
	...
	t.Run("Test find permission by valid ID", func(t *testing.T) {
		...
		roleCreated, _ := permissionRepo.StorePermission(permission)
		role, err := permissionRepo.GetPermissionById(roleCreated.ID)
		assert.NotNil(t, role)
		assert.NoError(t, err, err)
	})
	t.Run("Test find permission by random ID", func(t *testing.T) {
		randomUUID := uuid.New()
		_, err := permissionRepo.GetPermissionById(randomUUID)
		assert.Error(t, err)
	})
	t.Run("Test unique name constraint", func(t *testing.T) {
		permission, _ := domain.NewPermission("Unique", "Testing unique constraint")
		_, _ = permissionRepo.StorePermission(permission)
		permissionCreated, err := permissionRepo.StorePermission(permission)
		assert.Error(t, err)
